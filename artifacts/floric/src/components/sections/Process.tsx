  ...
  return (
    <section className="py-32 bg-secondary text-secondary-foreground relative">
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        >
          <span className="text-white/50 uppercase tracking-[0.2em] text-sm font-medium mb-4 block">{t.process.tag}</span>
...
function ProcessStep({ step, index }: { step: { num: string; title: string; desc: string }; index: number }) {
  ...
  return (
    ...
    >
      <div className="text-5xl md:text-6xl font-serif text-white/15 mb-6">{step.num}</div>
      ...
      {index < 4 && (
        <div className="hidden md:block absolute top-8 left-1/2 w-full h-[1px] bg-gradient-to-r from-white/20 to-transparent"></div>
      )}
    </div>
  );
}
